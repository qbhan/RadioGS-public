...
if __name__ == '__main__':
    ...
    for idx, frame in enumerate(tqdm(frames, leave=False)):
        ...
        gt_albedo = torch.from_numpy(gt_albedo_np[..., :3] * mask).permute(2, 0, 1).float().cuda()
        mask = torch.from_numpy(mask).permute(2, 0, 1).bool().cuda()
        mask = torch.logical_and(mask>0, (gt_albedo>0).all(dim=0, keepdim=True))

        # resize mask and albedo to match the image size
        mask = torch.nn.functional.interpolate(mask.unsqueeze(0).float(), size=(512, 512), mode='bilinear').squeeze(0).bool()
        gt_albedo = torch.nn.functional.interpolate(gt_albedo.unsqueeze(0), size=(512, 512), mode='bilinear', align_corners=False).squeeze(0)
        gt_albedo = srgb_to_rgb(gt_albedo) # convert to linear RGB
        ...
        # print(render_pkg['base_color_linear'].shape, gt_albedo.shape, mask.shape)
