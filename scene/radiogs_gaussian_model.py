...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, transmittance_min=0.03):
        ...
        # per-Gaussian ray tracing caches
        self.incident_directions = torch.empty(0)
    ...
    @property
    def get_incident_visibility(self):
        return self.incident_visibility
    
    @property
    def get_incident_directions(self):
        return self.incident_directions
    
    @property
    def get_incident_areas(self):
        return self.incident_areas

    @property
    def get_local_incident_radiance(self):
        return self.incident_radiance
    ...
    def update_lr_excluding_features(self, opt):
        for param_group in self.optimizer.param_groups:
            param_name = param_group.get("name", "")
            if param_name not in ["f_dc", "f_rest"]:
                if param_name == "xyz":
                    param_group["lr"] = opt.position_lr_init * self.spatial_lr_scale
                elif param_name == "opacity":
                    param_group["lr"] = opt.opacity_lr
                    ...
                    param_group["lr"] = opt.base_color_lr
                elif param_name == "roughness":
                    param_group["lr"] = opt.roughness_lr
                elif param_name == "metallic":
                    param_group["lr"] = opt.metallic_lr
                elif param_name == "env":
                    param_group["lr"] = opt.envmap_cubemap_lr
    ...
    def update_incidents_directions(self, incident_directions, incident_areas, mask=None):
        if mask is not None:
            ...
            self.incident_directions[mask] = incident_directions.detach()
            self.incident_areas[mask] = incident_areas.detach()
        else:
            self.incident_directions = incident_directions.detach()
            self.incident_areas = incident_areas.detach()

    def update_incident_radiance(self, incident_visibility, local_incident_radiance, detach=True, mask=None):
        if detach:
            if mask is not None:
                self.incident_visibility[mask] = incident_visibility.detach()
                self.incident_radiance[mask] = local_incident_radiance.detach()
            else:
                self.incident_visibility = incident_visibility.detach()
                self.incident_radiance = local_incident_radiance.detach()
        else:
            if mask is not None:
                self.incident_visibility[mask] = incident_visibility
                self.incident_radiance[mask] = local_incident_radiance
            else:
                self.incident_visibility = incident_visibility
    ...
    def precompute_incidents(self, light_t_min=0.0, only_vis=False, features=None, camera_center=None, back_culling=False, relight=False, f0=0.04):
        ...
        if relight:
            ...
            trace_wi = -self.incident_directions
            ...
            # Debug
            # print(f"fg_uv shape: {fg_uv.shape}, min: {fg_uv.min().item():.4f}, max: {fg_uv.max().item():.4f}")
            # print(f"FG_LUT shape: {self.FG_LUT.shape}")
            
            # Validate UV coordinates
            if torch.any(torch.isnan(fg_uv)) or torch.any(torch.isinf(fg_uv)):
                print("Warning: Invalid UV coordinates detected!")
                fg_uv = torch.nan_to_num(fg_uv, nan=0.0, posinf=1.0, neginf=0.0).clamp(0, 1)
            ...
            for i in range(0, fg_uv_flat.shape[0], batch_size):
                batch_uv = fg_uv_flat[i:i+batch_size]
                batch_fg = dr.texture(self.FG_LUT, batch_uv.reshape(1, -1, 1, 2).contiguous(), 
                                    filter_mode="linear", boundary_mode="clamp")
                fg_list.append(batch_fg.reshape(-1, 2))
            
            fg = torch.cat(fg_list, dim=0).reshape(*original_shape)
        ...
        self.incident_visibility = incident_visibility.detach()
        if only_vis: self.incident_radiance = torch.zeros_like(local_incident_lights)
        else: self.incident_radiance = local_incident_lights.detach()
